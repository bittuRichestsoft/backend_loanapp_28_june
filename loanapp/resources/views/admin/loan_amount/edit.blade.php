...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.loan_amount.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.loan_amount.update", [$loanAmount->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('range_from') ? 'has-error' : '' }}">
                <label for="name">{{ trans('global.loan_amount.fields.range_from') }}*</label>
                <input type="text" id="range_from" name="range_from" class="form-control" value="{{ old('name', isset($loanAmount) ? $loanAmount->range_from : '') }}">
            ...
            <div class="form-group {{ $errors->has('range_to') ? 'has-error' : '' }}">
                ...
                <input type="text" id="range_to" name="range_to" class="form-control" value="{{ old('name', isset($loanAmount) ? $loanAmount->range_to : '') }}">
            ...
            <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                ...
                <input type="text" id="amount" name="amount" class="form-control" value="{{ old('name', isset($loanAmount) ? $loanAmount->amount : '') }}">
