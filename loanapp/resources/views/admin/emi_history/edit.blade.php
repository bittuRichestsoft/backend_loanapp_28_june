...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.emi_history.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.emi_history.update", [$emiHistory->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('user_id') ? 'has-error' : '' }}">
                ...
                <select name="user_id" id="user_id" class="form-control select2">
                    @foreach($users as $id => $users)
                        <option value="{{ $users->id }}" {{ ($users->id==$emiHistory->user_id )?'selected':'' }}>
            ...
            <div class="form-group {{ $errors->has('loan_id') ? 'has-error' : '' }}">
                ...
                <select name="loan_id" id="loan_id" class="form-control select2" >
                    @foreach($loan_id as $id => $loan_id)
                        <option value="{{ $loan_id->id }}" {{ ($loan_id->id==$emiHistory->loan_id )?'selected':'' }}>
                            {{ $loan_id->amount }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('installment') ? 'has-error' : '' }}">
                ...
                <input type="text" id="installment" name="installment" class="form-control" value="{{ old('installment', isset($emiHistory) ? $emiHistory->installment : '') }}">
